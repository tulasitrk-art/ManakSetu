        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-2">
            <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full bg-maroon-800 text-amber-300 text-xs font-semibold">
              ...
              <span>{t("graph_badge", "Bureau of Indian Standards Knowledge Graph")}</span>
            </div>
            <h1 className="text-2xl sm:text-3xl font-bold font-serif">
              {t("graph_title", "Normative Standards & Hierarchy Visualizer")}
            </h1>
            <p className="text-xs sm:text-sm text-slate-300 max-w-2xl">
              {t("graph_subtitle", "Explore interconnected relationships between parent product standards, normative reference codes, mandatory testing protocols, and Central Ministry Quality Control Orders (QCOs).")}
            </p>
          </div>
          ...
          <form onSubmit={handleSearch} className="flex items-center space-x-2 w-full md:w-auto">
            <div className="relative flex-1 md:w-72">
              ...
              <input
                ...
                placeholder={t("search_graph_placeholder", "Search standard (e.g. IS 10322, IS 456)...")}
            ...
            >
              {t("reset_graph", "Reset Full Graph")}
            </button>
          </form>
        </div>
      </section>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-1 w-full space-y-6">
        {isLoading ? (
          <div className="h-96 flex flex-col items-center justify-center space-y-3 bg-white rounded-2xl border border-slate-200">
            ...
            <p className="text-xs text-slate-500 font-medium">{t("searching", "Computing graph topology & relations...")}</p>
          </div>
        ) : (
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 pb-3">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <span className={`px-2.5 py-1 rounded text-xs font-bold ${selectedNode.status === 'ACTIVE' ? 'bg-emerald-100 text-emerald-800' : 'bg-rose-100 text-rose-800'}`}>
                      {translateTerm(selectedNode.status || 'ACTIVE')}
                    </span>
                  </div>
                </div>
                <p className="text-sm text-slate-700">{selectedNode.label}</p>
                {selectedNode.division && (
                  <p className="text-xs text-slate-500"><strong>Division:</strong> {translateTerm(selectedNode.division)}</p>
                )}
              </div>
            )}
          </div>
