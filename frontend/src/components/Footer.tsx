    ...
    <footer className="bg-slate-900 text-slate-300 border-t border-slate-800 mt-20 no-print">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-12">
          ...
          <div className="md:col-span-2">
            ...
            <div className="flex items-center space-x-2 text-xs text-slate-400">
              ...
              <span>All Indian Standards synchronized with BIS Gazette Registry</span>
            </div>
          </div>
            ...
            <ul className="space-y-2.5 text-sm">
              <li>
                <Link href="/dashboard" className="hover:text-amber-400 transition-colors flex items-center gap-1.5">
                  <CheckCircle2 className="w-3.5 h-3.5 text-maroon-400" /> Hybrid Semantic Search
                </Link>
              </li>
              <li>
                <Link href="/tender-analyzer" className="hover:text-amber-400 transition-colors flex items-center gap-1.5">
                  <CheckCircle2 className="w-3.5 h-3.5 text-maroon-400" /> Tender PDF OCR Parser
                </Link>
              </li>
              <li>
                <Link href="/graph-view" className="hover:text-amber-400 transition-colors flex items-center gap-1.5">
                  <CheckCircle2 className="w-3.5 h-3.5 text-maroon-400" /> Normative Knowledge Graph
                </Link>
              </li>
              <li>
                <Link href="/qco-tracker" className="hover:text-amber-400 transition-colors flex items-center gap-1.5">
                  <CheckCircle2 className="w-3.5 h-3.5 text-maroon-400" /> Mandatory QCO Orders
                </Link>
              </li>
            </ul>
          </div>
        ...
        <div className="pt-8 border-t border-slate-800 text-xs text-slate-500 flex flex-col md:flex-row justify-between items-center gap-4">
          <p>© 2026 Department of Consumer Affairs, Ministry of Consumer Affairs, Food & Public Distribution, Govt. of India.</p>
          <div className="flex items-center space-x-4">
            <span>Official Bureau of Indian Standards (BIS) Recommendation Portal</span>
            <span>•</span>
            <span className="text-maroon-400 font-mono">v1.0.0 Production Release</span>
          </div>
        </div>
      </div>
    </footer>
